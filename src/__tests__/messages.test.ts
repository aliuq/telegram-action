...
describe("splitTelegramMessageChunks", () => {
  ...
  test("splits oversized code block into multiple chunks", () => {
    const longCode = Array.from({ length: 500 }, (_, i) => `const variable${i} = "value_${i}_${"x".repeat(20)}";`).join(
      "\n",
    );
    const message = `\`\`\`js\n${longCode}\n\`\`\``;
    const chunks = splitTelegramMessageChunks(message, TELEGRAM_MESSAGE_LIMIT);
  ...
  test("prefers natural split boundaries", () => {
    const paragraphs = Array.from({ length: 30 }, (_, i) => `Paragraph ${i}: ${"content ".repeat(40)}`).join("\n\n");
    ...
    for (const chunk of chunks.slice(0, -1)) {
      const endsNaturally = chunk.raw.endsWith("\n\n") || chunk.raw.endsWith("\n") || chunk.raw.endsWith(" ");
...
describe("buildStreamingFrames", () => {
  ...
  test("generates at least minFrames for normal text", () => {
    const message = "Hello world, this is a test message with enough content to generate multiple frames.";
